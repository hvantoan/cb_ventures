...
import { Row, Col, Form, Input, Select, Table, DatePicker, Radio, Upload, Spin } from 'antd'
...
import { axiosDataUpdate, axiosFileUploder, axiosDataSingle, axiosDataRead } from '@/redux/crud/axios/actionCreator'
...
function Edit() {
  ...
  const { crud, isLoading, url, isFileLoading } = useSelector((state: any) => {
    return {
        ...
        name: (
          <div className="flex items-center">
            <figure className="mx-2 mb-0">
              <img
                className="w-10"
                src={image ? `${process.env.NEXT_PUBLIC_API_ENDPOINT}/${image}` : '/img/avatar/profileImage.png'}
                alt={id}
              />
            </figure>
            <figcaption>
