...
import { UilEye, UilEdit, UilTrashAlt, UilEllipsisH } from '@iconscout/react-unicons'
...
function FileListCard({ title }: any) {
  return (
        ...
        {filesData.map((file, index) => (
          <div className="flex items-center justify-between file-list__single" key={index}>
            <div className="flex items-center w-[50%] gap-x-[16px]">
              <img className="max-w-[42px]" src={`/img/files/${file.logo}`} alt="File Logo" />
              <div>
                ...
                <span className="inline-flex text-[12px] leading-[1.42] gap-[10px]">
                  {file.actions.map((action, ind) => (
                    <Link className="font-medium text-primary" href={action.url} key={ind}>
                      {action.label}
                    </Link>
                  ))}
                </span>
