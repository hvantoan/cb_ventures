...
function BlogDetails() {
  ...
  const post: any = BlogCardData.find((item) => item.slug === slug)

  const { title, img, content, author, authorImg, category, postDate }: any = post
      ...
      <main className="min-h-[715px] lg:min-h-[580px] bg-transparent px-[30px] xl:px-[15px] pb-5 xl:pb-0">
        <Row justify="center">
          <Col lg={15}>
            <>
              <div className="mb-20">
                <img className="rounded-10" src={`/img/blogs/blog-details/${img}`} alt={title} />
                  ...
                  <div>
                    ...
                    <ul className="flex flex-wrap items-center mb-11 lg:mb-8 md:mb-5">
                      <li className="relative flex items-center gap-x-2.5 m-1 text-light dark:text-white/60 text-15">
                        <img className="max-w-[50px]" src={`/img/users/${authorImg}`} alt={author} />
