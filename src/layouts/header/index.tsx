...
const HeaderTop = () => {
  ...
  const { rtlData, mode, topMenu, menuCollapse } = useAppSelector((state) => state.layout);
            ...
            <div className="flex items-center justify-between">
              <Link href="/">
                <Image
                  ...
                  src={mode === 'lightMode' ? '/img/logo_horizontal.png' : '/img/logo_horizontal.png'}
                  alt="Logo"
                  width="140"
                  height="20"
                />
              ...
              {!topMenu || (typeof window !== 'undefined' && window.innerWidth <= 1200) ? (
                ...
                >
                  <ReactSVG src={`/img/icon/left-bar.svg`} className="[&>div>svg]:h-[20px] [&>div>svg]:w-[20px]" />
                </button>
              ) : null}
            </div>
          </div>
