...
import topProduct from '@/data/table-data.json';
...
const TopSellingProduct = React.memo(() => {
    ...
    const sellingData: Array<{ key: number; name: any; img?: any; price: any; sold: any; revenue: any }> = [];
    if (topSaleProduct !== null && topSaleProduct[sellingTab as keyof sellingData]) {
        topSaleProduct[sellingTab as keyof sellingData].map((value: any, index: number) => {
            ...
            return sellingData.push({
                ...
                price: (
                    <span className="font-normal capitalize text-[14px] text-dark dark:text-white/[.87]">{price}</span>
                ),
                sold: (
                    <span className="font-normal capitalize text-[14px] text-dark dark:text-white/[.87]">{sold}</span>
                ),
                revenue: (
