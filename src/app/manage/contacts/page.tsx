...
function ContactTable() {
  ...
  const users = useAppSelector((state) => state.contact.data);
  ...
  const handleEditOk = (values: Contact) => {
    onCancel();
    const updateUsers: Contact[] = users;
  ...
  sortData
    ...
    .map((user: Contact) => {
      ...
      return usersTableData.push({
        ...
        email: <span className="text-body dark:text-white/60 text-[15px] font-medium">{email}</span>,
        ...
        phone: <span className="text-body dark:text-white/60 text-[15px] font-medium">{phone}</span>,
        action: (
          <div className="min-w-[150px] text-end -m-2">
            ...
            >
              <UilStar className={`w-[14px] h-[14px] mx-auto ${stared ? 'text-warning' : 'text-light-extra'}`} />
            </Buttons>
            <DropDown
