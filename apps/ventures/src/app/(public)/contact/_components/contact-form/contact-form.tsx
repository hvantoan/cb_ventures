...
const ContactForm: React.FC<ContactFormProps> = ({ formId }) => {
  const { data: contact } = useQueryMerchant();
  const theme = useTheme();

  const { control, handleSubmit, reset } = useForm({ resolver, defaultValues: new Contact() });
      ...
      >
        <div className='grid grid-cols-1 gap-2 md:grid-cols-2 md:gap-4'>
          <Controller
            ...
            render={({ field: { value, onChange } }) => (
              <TextField label={contactFormLabels.name} value={value ?? ''} onChange={onChange} sx={textFieldSx} />
            )}
          />
          <Controller
            control={control}
        ...
        <div className='grid grid-cols-1 gap-2 md:grid-cols-2 md:gap-4'>
          <Controller
            ...
            render={({ field: { value, onChange } }) => (
              <TextField
                label={contactFormLabels.bankAccount}
                InputProps={{ readOnly: false }}
                value={value ?? ''}
                onChange={onChange}
                sx={textFieldSx}
          ...
          <Controller
            ...
            render={({ field: { value, onChange } }) => (
              <TextField
                label={contactFormLabels.bankName}
                InputProps={{ readOnly: false }}
                value={value ?? ''}
                onChange={onChange}
                sx={textFieldSx}
          ...
          <Controller
            ...
            render={({ field: { value, onChange } }) => (
              <TextField
                label={contactFormLabels.bankBranch}
                InputProps={{ readOnly: false }}
                value={value ?? ''}
                onChange={onChange}
                sx={textFieldSx}
